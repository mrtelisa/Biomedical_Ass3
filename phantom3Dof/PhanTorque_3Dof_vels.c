/*PhanTorque_3Dof_vels.c. Code of the s-function part of Phan3DoF Library.
...
#define S_FUNCTION_NAME  PhanTorque_3Dof_vels
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    ssSetOutputPortWidth(S, 2, 3);  //Lin_vel (vx,vy,vz)
    ssSetOutputPortWidth(S, 3, 16); //Homogeneous Transformation (H)
...
HDCallbackCode HDCALLBACK SchedulerCallback(void *pUserData)
{
    ...
    hdGetDoublev(HD_CURRENT_VELOCITY, Haptic.velocity);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    real_T       *y_velocity = ssGetOutputPortSignal(S, 2);
    real_T       *y_transform = ssGetOutputPortSignal(S, 3);
    
    for (i=0; i<3; i++)
    {
        y_joint_angle[i] = Haptic.jointAngles[i];
        y_position[i] =(Haptic.position[i])*.001;
        y_velocity[i] =(Haptic.velocity[i])*.001;
    }
